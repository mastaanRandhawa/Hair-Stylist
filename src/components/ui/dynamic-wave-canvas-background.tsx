...
export const HeroWave = memo(function HeroWave({ className }: HeroWaveProps) {
  ...
  useEffect(() => {
    ...
    const drawStaticBackground = () => {
      ...
      gradient.addColorStop(0.7, "#3d3520");
    ...
    const render = (now: number) => {
      ...
      for (let y = 0; y < h; y++) {
        ...
        for (let x = 0; x < w; x++) {
          ...
          const r = Math.max(0, Math.min(1, 0.05 + goldMix * 0.82)) * intensity;
          const g = Math.max(0, Math.min(1, 0.04 + goldMix * 0.68)) * intensity;
          const b = Math.max(0, Math.min(1, 0.03 + goldMix * 0.14)) * intensity;
